...
### [​](https://code.claude.com/docs/en/mcp#automatic-reconnection) Automatic reconnection
...
Create an app through the server’s developer portal and note your client ID and client secret. Many servers also require a redirect URI. If so, choose a port and register a redirect URI in the format `http://localhost:PORT/callback`. Use that same port with `--callback-port` in the next step. 2
...
### [​](https://code.claude.com/docs/en/mcp#restrict-oauth-scopes) Restrict OAuth scopes
...
After running the command, you’ll see an interactive dialog that allows you to select which servers you want to import. 3
...
Add servers at [claude.ai/settings/connectors](https://claude.ai/settings/connectors). On Team and Enterprise plans, only admins can add servers. 2

Authenticate the MCP server

Complete any required authentication steps in Claude.ai. 3
...
### [​](https://code.claude.com/docs/en/mcp#raise-the-limit-for-a-specific-tool) Raise the limit for a specific tool
...
Type `@` in your prompt to see available resources from all connected MCP servers. Resources appear alongside files in the autocomplete menu. 2
...
If you prefer threshold-based loading, set `ENABLE_TOOL_SEARCH=auto` to load schemas upfront when they fit within 10% of the context window and defer only the overflow. See [Configure tool search](https://code.claude.com/docs/en/mcp#configure-tool-search) for all options.
...
Type `/` to see all available commands, including those from MCP servers. MCP prompts appear with the format `/mcp__servername__promptname`. 2
...
**When using `managed-mcp.json`**: Users cannot add MCP servers through `claude mcp add` or configuration files. The `allowedMcpServers` and `deniedMcpServers` settings still apply to filter which managed servers are actually loaded.[Claude Code Docs home page](https://code.claude.com/docs/en/overview)

[Privacy choices](https://code.claude.com/docs/en/mcp#)
