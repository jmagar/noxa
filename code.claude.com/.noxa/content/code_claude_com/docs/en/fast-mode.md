...
By default, fast mode persists across sessions. Administrators can configure fast mode to reset each session. See [require per-session opt-in](https://code.claude.com/docs/en/fast-mode#require-per-session-opt-in) for details.
For the best cost efficiency, enable fast mode at the start of a session rather than switching mid-conversation. See [understand the cost tradeoff](https://code.claude.com/docs/en/fast-mode#understand-the-cost-tradeoff) for details.
...
- [Status line configuration](https://code.claude.com/docs/en/statusline): display model and context information[Claude Code Docs home page](https://code.claude.com/docs/en/overview)

[Privacy choices](https://code.claude.com/docs/en/fast-mode#)
