...
# The generated .env is broader than config/.env.example: it includes local
...
configure_env() {
    ...
    if [[ -f "$SCRIPT_DIR/proxies.txt" ]]; then
        ...
        printf "    ${DIM}cp config/proxies.example.txt proxies.txt${RESET}\n"
    fi
    local proxy_file=""
    ...
    # Write .env
    # config/.env.example covers the runtime noxa variables; this file adds local
